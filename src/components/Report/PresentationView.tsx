      ...
      {justification && (
        <div 
          className={`transition-all duration-700 ease-out transform ${
            sliderPos < 50 ? 'opacity-100 translate-y-0' : 'opacity-20 translate-y-2'
          }`}
        >
            ...
            {steps.map((step, i) => (
              <button
                ...
                className={`px-4 py-2 rounded-full text-[9px] font-black uppercase tracking-widest transition-all ${
                  activeStep === i ? 'bg-psi-gold text-obsidian' : 'text-slate-500 hover:text-white'
                }`}
              >
                {step.title}
              </button>
               ...
               <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
                 {project.uiVault?.slice(0, 8).map((comp: UIComponent) => (
                    <div key={comp.id} className="glass-card p-8 group hover:border-psi-gold/40 transition-all text-center rounded-[2rem]">
                       ...
                       <h4 className="text-[11px] font-black uppercase tracking-widest text-white mb-2">{comp.name}</h4>
                       <span className="px-3 py-1 bg-white/5 border border-white/10 rounded-full text-[7px] font-black uppercase text-slate-500 tracking-widest">{comp.category}</span>
                    </div>
                 )) || (
