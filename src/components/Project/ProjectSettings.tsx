...
export const ProjectSettings: React.FC<ProjectSettingsProps> = ({
  ...
  const TabButton = ({ id, icon: Icon, label }: { id: SettingsTab, icon: any, label: string }) => (
    <button
      ...
      className={`flex items-center gap-2 pb-4 text-[10px] font-bold uppercase tracking-wide border-b-2 transition-all ${activeTab === id ? 'text-primary border-primary' : 'text-white border-transparent hover:text-white'
        }`}
    >
      <Icon size={14} />
      {safeString(label)}
    ...
    <div className="flex-1 overflow-y-auto custom-scrollbar p-12 bg-background animate-in fade-in duration-500">
      <div className="max-w-6xl mx-auto space-y-12 pb-32">
        ...
        <header className="flex flex-col gap-8 border-b border-border pb-8">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-6">
              <div
                className="w-24 h-24 rounded-lg bg-surface flex items-center justify-center border border-border shadow-sm overflow-hidden cursor-pointer group hover:border-primary transition-all"
              ...
              <div>
                <h2 className="text-4xl font-bold text-white tracking-tight uppercase">{safeString(project.name || "UNNAMED PROJECT")}</h2>
                <p className="text-white text-[10px] font-medium uppercase tracking-[0.4em] mt-1">Sovereign Command Center</p>
              </div>
            </div>
            ...
            <div className="flex gap-4">
              {!readiness.isReady && (
                <div className="flex flex-col items-end justify-center mr-4">
                  ...
                  <p className="text-[10px] font-bold text-white uppercase">{safeString((readiness.missing || []).length)} Prerequisites Pending</p>
                </div>
              )}
              <button
                onClick={handleExtractDNA}
                disabled={isExtractingDNA || !project.brand?.logo}
                className={`flex items-center gap-3 px-6 py-3 rounded-lg text-[10px] font-bold uppercase tracking-widest shadow-sm hover:scale-105 transition-all disabled:opacity-50 ${project.brand?.dna ? 'bg-surface text-primary border border-primary/30' : 'bg-primary text-white'}`}
              >
        ...
        <main className="min-h-[500px]">
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-12">
                <div className="space-y-10">
                  <div className="space-y-4">
                    <label className="text-[10px] font-bold uppercase tracking-widest text-white flex items-center gap-2">
                    ...
                    <textarea
                      ...
                      className="w-full bg-surface border border-border rounded-lg p-6 text-sm text-white min-h-[140px] focus:border-primary outline-none transition-all placeholder:text-slate-400"
                ...
                <div className="space-y-10">
                  <div className="bg-surface border border-border p-10 space-y-8 rounded-lg">
                    ...
                    <div
                      className="border-2 border-dashed border-border rounded-lg p-16 text-center hover:border-primary/50 transition-all cursor-pointer group"
                      onClick={() => guidelinesInputRef.current?.click()}
                    >
                      <Upload size={48} className="mx-auto text-white mb-6 group-hover:text-primary transition-colors" />
                      <p className="text-[10px] font-bold text-white uppercase tracking-widest">Upload Brand Identity Kit (PDF)</p>
                    ...
                    {project.brand?.dna ? (
                      <div className="space-y-6 pt-4">
                        ...
                        <h5 className="text-[9px] font-bold uppercase text-white tracking-widest">Extracted Design Tokens</h5>
                        <div className="grid grid-cols-2 gap-4">
                          <div className="p-4 bg-background rounded-lg border border-border">
                            <p className="text-[7px] font-bold text-white uppercase mb-1">Theme</p>
                          ...
                          <div className="p-4 bg-background rounded-lg border border-border">
                            <p className="text-[7px] font-bold text-white uppercase mb-1">Primary Color</p>
                            <p className="text-[10px] text-white font-bold truncate">{safeString(project.brand.dna?.colors?.primary || "#FFFFFF")}</p>
                          </div>
                        </div>
                      </div>
                    ) : (
                      <p className="text-[9px] text-white uppercase italic text-center">Sync Logo DNA to extract high-fidelity tokens.</p>
                    )}
                  </div>
                </div>
              </div>
          ...
          {activeTab === 'market' && (
            <div className="space-y-12 animate-in slide-in-from-right duration-500">
              <div className="flex justify-between items-center bg-primary/5 border border-primary/20 p-8 rounded-lg">
                <div className="flex gap-4 items-center">
                  <div className="w-12 h-12 bg-primary/10 rounded-lg flex items-center justify-center text-primary">
                    <Target size={24} />
                  </div>
                  <div>
                    <h4 className="text-sm font-bold uppercase tracking-widest text-white">Adversary Intelligence Mapping</h4>
                    <p className="text-[9px] text-white uppercase font-bold">Grouped competitor URLs for batch benchmarking.</p>
                  </div>
                </div>
                <button
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-12">
                <div className="space-y-8">
                  <div className="flex justify-between items-center border-b border-border pb-4">
                    <h4 className="text-[11px] font-bold uppercase tracking-widest text-white">Direct Competitors (URLs)</h4>
                  ...
                  <div className="space-y-4">
                    {(project.competitors || []).map((url, idx) => (
                      <div key={idx} className="flex gap-3">
                        <input
                          ...
                          className="flex-1 bg-surface border border-border rounded-lg px-4 py-3 text-xs text-white focus:border-primary outline-none transition-all"
                    ...
                    {(project.competitors || []).length === 0 && (
                      <p className="text-[9px] text-white uppercase italic py-4 text-center border border-dashed border-border rounded-lg">No competitors mapped.</p>
                    )}
                  </div>
                </div>
                ...
                <div className="space-y-8">
                  <div className="flex justify-between items-center border-b border-border pb-4">
                    <h4 className="text-[11px] font-bold uppercase tracking-widest text-white">Reference Aspiration URLs</h4>
                  ...
                  <div className="space-y-4">
                    ...
                    {(project.inspirationUrls || []).length === 0 && (
                      <p className="text-[9px] text-white uppercase italic py-4 text-center border border-dashed border-border rounded-lg">No inspiration references added.</p>
                    )}
                  </div>
                </div>
              </div>
          ...
          {activeTab === 'analytics' && (
            <div className="space-y-12 animate-in slide-in-from-right duration-500">
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-12">
                <div className="bg-surface border border-border p-12 space-y-10 rounded-lg">
                  <div className="flex items-center gap-4">
                    <div className="w-12 h-12 bg-emerald-500/10 rounded-lg flex items-center justify-center text-emerald-500">
                      <UserCheck size={28} />
                    </div>
                    <h4 className="text-2xl font-bold uppercase tracking-tight text-white">Persona Builder</h4>
                  </div>
                  <div className="space-y-4">
                    <p className="text-[10px] text-white uppercase tracking-widest">Target Demographic Profiles</p>
                    <textarea
                      ...
                      className="w-full bg-background border border-border rounded-lg p-8 text-sm text-white min-h-[220px] outline-none focus:border-primary transition-all"
...
• First-time luxury buyers..."
                ...
                <div className="bg-surface border border-border p-12 space-y-10 rounded-lg">
                  <div className="flex items-center gap-4">
                    ...
                    <h4 className="text-2xl font-bold uppercase tracking-tight text-white">Strategic KPIs</h4>
                  </div>
                  <div className="space-y-4">
                    <p className="text-[10px] text-white uppercase tracking-widest">Business Success Thresholds</p>
                    <textarea
          ...
          {activeTab === 'standards' && (
            <div className="space-y-12 animate-in slide-in-from-right duration-500">
              <div className="flex items-center justify-between">
                <div>
                  <h3 className="text-3xl font-bold text-white uppercase tracking-tight">Niche Intelligence: {safeString(project.industry || "General")}</h3>
                  <p className="text-[10px] text-white font-bold uppercase tracking-[0.4em] mt-2">Cross-referencing domain against Platinum Sector Benchmarks</p>
                </div>
                <div className="px-6 py-3 bg-emerald-500/10 border border-emerald-500/20 rounded-lg flex items-center gap-3">
                  ...
                  <div key={gIdx} className="space-y-6">
                    ...
                    <div className="space-y-4">
                      {(group.items || []).map((item) => (
                        <div key={safeString(item.id)} className="bg-surface border border-border p-5 rounded-lg group/item hover:border-primary/30 transition-all">
                          ...
                          <p className="text-[9px] text-white leading-tight group-hover/item:text-white transition-colors">{safeString(item.description || "No description provided.")}</p>
                        </div>
                      ))}
                    </div>
                  </div>
              ...
              <section className="bg-surface border border-dashed border-border p-10 rounded-lg flex items-center justify-between">
                <div className="flex gap-6 items-center">
                  ...
                  <div>
                    <h4 className="text-lg font-bold text-white uppercase tracking-tight">Compliance ROI Narrative</h4>
                    <p className="text-[10px] text-white uppercase tracking-widest">Filling niche gaps adds approximately <strong>+18%</strong> to predicted conversion rate.</p>
                  </div>
                </div>
                <button className="px-8 py-3 bg-white/5 border border-border text-white rounded-lg text-[9px] font-bold uppercase tracking-widest hover:bg-white/10 transition-all">Refine Niche Standard</button>
              </section>
            </div>
          )}
          ...
          {activeTab === 'architecture' && (
            <div className="space-y-10 animate-in slide-in-from-right duration-500">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-10">
                <div className="space-y-4">
                  <label className="text-[10px] font-bold uppercase tracking-widest text-white flex items-center gap-2"><Globe size={14} /> Domain Root</label>
                  <input
                    type="text"
                    value={safeString(project.baseUrl || "")}
                    disabled
                    className="w-full bg-surface border border-border rounded-lg p-5 text-white font-mono text-sm cursor-not-allowed"
                  />
                </div>
                <div className="space-y-4">
                  <label className="text-[10px] font-bold uppercase tracking-widest text-white flex items-center gap-2"><Briefcase size={14} /> Industry Sector</label>
                  <input
                    type="text"
                    value={safeString(project.industry || "")}
                    onChange={(e) => onUpdate({ industry: e.target.value })}
                    className="w-full bg-surface border border-border rounded-lg p-5 text-white focus:border-primary outline-none transition-all"
                  />
                </div>
              </div>

              <section className="bg-surface border border-border p-12 text-center space-y-8 rounded-lg">
                <div className="w-16 h-16 bg-primary/10 rounded-lg mx-auto flex items-center justify-center text-primary">
                  <BarChart3 size={32} />
                </div>
                <div className="space-y-3">
                  <h3 className="text-2xl font-bold text-white uppercase tracking-tight">Executive Transformation Summary</h3>
                  <p className="text-sm text-white max-w-md mx-auto leading-relaxed">Compile current intelligence into a high-fidelity client presentation.</p>
                </div>
                <div className="flex justify-center gap-4">
                  <button onClick={() => setShowReportPreview(true)} className="px-10 py-4 bg-background border border-border text-white rounded-lg text-[10px] font-bold uppercase tracking-widest hover:bg-secondary transition-all">Preview Mode</button>
                  <button onClick={handleExportReport} className="px-10 py-4 bg-primary text-white rounded-lg text-[10px] font-bold uppercase tracking-widest shadow-md hover:bg-primary-hover transition-all">Generate Package</button>
                </div>
              </section>
            </div>
          )}
          ...
          {activeTab === 'status' && (
            <div className="space-y-10 animate-in slide-in-from-right duration-500">
              <div className="flex items-center justify-between border-b border-border pb-10">
                <div>
                  <h4 className="text-3xl font-bold text-white uppercase tracking-tight">Project Integrity Status</h4>
                  <p className="text-[10px] text-white font-bold uppercase tracking-[0.4em] mt-2">Prerequisites for Phase 1 Architectural Analysis</p>
                </div>
                <div className="text-right">
                  ...
                  <p className="text-[9px] font-bold text-white uppercase tracking-widest">Audit Stability Score</p>
                </div>
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {(project.checklist || []).map((item) => (
                  <div
                    ...
                    className={`p-8 rounded-lg border cursor-pointer transition-all flex flex-col gap-6 group ${item.completed ? 'bg-emerald-500/5 border-emerald-500/20' : 'bg-surface border-border hover:border-primary/40'
                      }`}
                  >
                    <div className="flex items-center justify-between">
                      <span className="text-[8px] font-bold uppercase text-white tracking-[0.3em]">{safeString(item.category || "General")}</span>
                      <div className={`w-8 h-8 rounded-lg border-2 flex items-center justify-center transition-all ${item.completed ? 'bg-emerald-500 border-emerald-500 text-white shadow-sm' : 'border-border'
                        }`}>
                        {item.completed && <Zap size={16} className="fill-current" />}
                      </div>
                    </div>
                    <p className={`text-sm font-bold leading-tight ${item.completed ? 'text-emerald-600' : 'text-white'}`}>{safeString(item.label || "Untitled Task")}</p>
                  </div>
                ))}
              </div>
            </div>
